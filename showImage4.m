...
if print
   ...
   %{ 
% 保存为高质量 PNG 格式
        png_file = sprintf('Outputs/%d.png', id);
        dpi = 300; % 分辨率
        figure('Visible', 'off'); % 创建隐藏图形窗口
        imshow(IMN, 'InitialMagnification', 'fit');
        set(gca, 'LooseInset', max(get(gca, 'TightInset'), 0.02)); % 去除多余白边
        print(gcf, png_file, '-dpng', ['-r', num2str(dpi)]); % 保存为 PNG
        close(gcf); % 关闭图形窗口
   %}
end
